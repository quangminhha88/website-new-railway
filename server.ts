 ...
 * Serves Vite frontend (dist/) + API routes
 */
...
async function callCron(endpoint: string) {
  try {
    ...
    console.log(`✅ Cron executed: ${endpoint}`);
  } catch (error) {
    console.error(`❌ Cron failed: ${endpoint}`, error);
  }
}

// Daily at 03:00 UTC
cron.schedule('0 3 * * *', () => {
  callCron('/api/cron/daily');
});

// Monday at 04:00 UTC
cron.schedule('0 4 * * 1', () => {
  callCron('/api/cron/promote-ab');
});

// Daily at 05:00 UTC
cron.schedule('0 5 * * *', () => {
  callCron('/api/cron/refresh-content');
});
...
// ../dist => correct frontend build folder
